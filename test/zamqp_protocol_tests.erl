...
%% Defines
-define(BASIC, 60).
-define(SERVERHOST, <<"myserver.com">>).
%% ===================================================================
%% Tests.
%% ===================================================================
    ...
    [
     [{"heartbeat",
       ?_test(?assertMatch({ok, #{frame := heartbeat}, _},
                           zamqp_protocol:decode(
                             iolist_to_binary(
                               zamqp_protocol:encode(heartbeat, none)))))}],
    ...
    [
     ...
     {"connection start",
      ?_test(
           ?assertMatch({ok,
                        ...
                        zamqp_protocol:decode(
                          iolist_to_binary(
                            zamqp_protocol:encode(connection,
                                                 start,
                                                 #{server_properties =>
                                                       ServerProperties})))))},
     [{"connection " ++ atom_to_list(Method),
     ...
     [{"exchange " ++ atom_to_list(Method),
       ?_test(?assertMatch({ok,
                           ...
                           zamqp_protocol:decode(
                             iolist_to_binary(
                               zamqp_protocol:encode(exchange,
                                                    Method,
                                                    #{channel => 4711,
                                                      exchange => <<"mix">>
                                                     })))))}
